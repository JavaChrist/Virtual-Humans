...
Le chemin image 11A n’envoie pas les chaînes overlay au provider (`phase-11a-image-prompt-v2`). Le variant image est uniquement visuel (`113_`). La typographie est un dérivé déterministe post-ingest (`111_`). Runtime Production **`60cc335`**. Execution (`118_`) : composed `6a2beca9…`. Human Review (`119_`) : composé REJECT. Diagnostic (`120_`) : atlas `shapes-v1` · composeur **1.1.0 local non déployé** · parent `7832765d…` conservé.
