...
Voice/TTS 11C (`140_`–`147_`) : wiring + binding + catalogue + preflight + apply remote + grant hardening + apply (`phase-11c-voice-identity-grant-hardening-remote-apply.test.ts`) ; migrations-static **32/32** ; **0** ElevenLabs / seed / flags. VoiceId jamais dans les assertions. DB locale N/A le 15 août.
