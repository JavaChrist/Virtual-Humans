...
Voice/TTS 11C (`140_`–`153_`) : wiring + catalogue + grants + seed + binding + live preflight + first paid guards (`phase-11c-voice-tts-first-paid-execution.test.ts`) ; migrations-static **32/32** ; **1** ElevenLabs réel hors suite ; flags OFF. VoiceId jamais dans les assertions. DB locale N/A le 16 août.
