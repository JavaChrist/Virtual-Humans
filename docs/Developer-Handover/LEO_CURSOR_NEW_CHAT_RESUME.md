...
Nature : living resume. `172_` app-update preflight **consommée**. RideCloud apply **suspendu**. Voir living handover.
...
- créer `GET /api/version` public read-only selon `172_`
- étendre `PwaRegister` (poll, Plus tard, blockers, BroadcastChannel)
- tracing `../SDK_VERSION` borné · allowlist proxy
- tests · **aucun** deploy · **aucun** flag · **ne pas** réécrire `sw.js`

RideCloud apply **`AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_REMOTE_APPLY_ONCE_NO_PROVIDER`** reste **suspendue**.

Interdit : provider · dépense · média Git · lecture/upload pack · RPC bind · persist bind · run/job/attempt/output · activation · lipsync · merge/export · publication · TTS · flag write · deploy · duplication PWA.
...
Lis entièrement docs/Developer-Handover/LEO_CURSOR_NEW_CHAT_RESUME.md, puis CURRENT_STATE_AND_RESUME.md et le rapport 172_PHASE_VHS_APP_UPDATE_VERSIONING_AND_NOTIFICATION_PREFLIGHT.md.
...
La porte active est AUTH_VHS_APP_UPDATE_VERSIONING_AND_NOTIFICATION_IMPLEMENT_NO_DEPLOY_NO_FLAG_WRITE. Elle n’est pas encore exécutée. RideCloud apply est suspendue.

App update preflight READY (172_). UI parity deploy READY (171_). SHA e4703bf applicatif. Alias docs c808fa2. Cartes dashboard COMMITTED (170_). SDK tracing READY (169_). RideCloud bind kind schema remote READY (167_) apply suspendu. Migration locale 33e non appliquée. Bind preflight READY (165_). Projet CREATED (164_). Pack 158_ + 5 variantes HD 159_. Storyboard 26 s. Auth 172_ / 171_ / 170_ / 169_ / 168_ / 167_ / 166_ / 165_ / 164_ / 163_ / 162_ / 161_ / 160_ / 159_ / 158_ / 157_ / 156_ / 155_ / 153_ consommées. Aucun provider. 0¢. N’invente aucun claim. Aucun média Git. Aucun apply. Aucun deploy sans Auth. Aucun flag write.
...
Lis entièrement docs/Developer-Handover/LEO_CURSOR_NEW_CHAT_RESUME.md, puis CURRENT_STATE_AND_RESUME.md, .cursor/rules/living-handover.mdc et 172_PHASE_VHS_APP_UPDATE_VERSIONING_AND_NOTIFICATION_PREFLIGHT.md.
...
Ne commence aucune porte sans prompt Auth explicite de Léo/Christian dans CE chat. Ne rejoue pas 153_, 155_, 156_, 157_, 158_, 159_, 160_, 161_, 162_, 163_, 164_, 165_, 166_, 167_, 168_, 169_, 170_, 171_ ni 172_. Aucun provider. 0¢. Aucun apply. Aucun persist bind. Aucun média Git. Aucun deploy sans Auth. Aucun flag write. N’invente aucun claim.

La prochaine porte est AUTH_VHS_APP_UPDATE_VERSIONING_AND_NOTIFICATION_IMPLEMENT_NO_DEPLOY_NO_FLAG_WRITE. Elle n’est pas autorisée par ce fichier de reprise. 0 deploy. 0 flag. Ne pas réécrire sw.js. RideCloud apply reste suspendue.
```
...
- [ ] Lire le dernier STOP (ce fichier + living handover + `172_`)
...
- [ ] Confirmer prochaine Auth = `AUTH_VHS_APP_UPDATE_VERSIONING_AND_NOTIFICATION_IMPLEMENT_NO_DEPLOY_NO_FLAG_WRITE`
