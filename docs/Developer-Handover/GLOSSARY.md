...
**Provider text policy no_text** — le prompt OpenAI Image v2 interdit lettres/mots/chiffres/UI textuelle ; overlay déterministe `WIRED_DISABLED` ; copy overlay retiré (`113_`) ; paid `115_` **COMPOSITOR_FAILED_NO_RETRY** (`png: unsupported filter`) ; décodeur filtres 0–4 livré (`116_`) ; preflight compose `117_` ; execution `118_` **COMPOSED_ASSET_PRIVATE_HUMAN_REVIEW_PENDING**.
**Phase 11A PNG filter decoder** — `decodeRgbPng` interne (zlib Node, pas `sharp`) : filtres 0–4, RGB 8-bit non interlacé, CRC, bornes 1024 / 8 MiB ; RGBA/gray/indexed/Adam7 rejetés (`116_`). Asset provider réel : filtres rencontrés **1–4** (`117_`). Composed Production : `6a2beca9…` (`118_`).
