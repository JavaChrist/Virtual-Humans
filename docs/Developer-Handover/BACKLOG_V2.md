...
Motion     : ARCHITECTURE_READY · MT-001…012 IMPLEMENTED · MT-013A…E DONE
             ...
             MT-013G MEDIA_INVALID (`85_`) — source 16s/360p ; identity OK
...
Next major : source 3s≥720p → re-validate → upload → deploy/flags → paid MV-001
```
...
- **Motion / Performance Transfer** 🟡 Architecture `59_` · **MT-001…012 IMPLEMENTED** (`60_`…`72_`) · **MT-013A…F** (`73_`…`84_`) · **MT-013G** `MEDIA_INVALID` (`85_`) · fal adapter + worker **disabled** · synthetic E2E ready · privacy **ACCEPTED_LIMITED_MV001** · Production → unavailable · **MV001_NOT_EXECUTED** · **NO PAID BENCHMARK_YET**.
...
- **Prochaine porte majeure** : **source 3s ≥720p** → **re-validate MT-013G** → **private upload** → **deploy/flags** → **paid MV-001** — ne pas fusionner.
