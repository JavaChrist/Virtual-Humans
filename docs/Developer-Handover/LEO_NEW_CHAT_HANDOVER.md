...
Appliquer **exactement une** migration ACL. **Aucun** seed. **Aucun** second apply. **Aucun** appel ElevenLabs. **Aucun** lipsync.

## Première action du nouveau chat

1. Lire entièrement `CURRENT_STATE_AND_RESUME.md`.
2. Lire ce fichier.
3. Recevoir le prochain rapport STOP de Cursor concernant `AUTH_11C_VOICE_IDENTITY_CATALOG_GRANT_HARDENING_REMOTE_APPLY_ONCE`.
4. Ne pas refaire les phases déjà terminées.
5. Vérifier le rapport et préparer la prochaine porte.

Si `146_` est STOP, la porte suivante est l’apply unique des grants (sans seed). Aucun second submit I2V.
