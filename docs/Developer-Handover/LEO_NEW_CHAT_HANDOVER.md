...
Voice/TTS `/director` est **WIRED_DISABLED** (`140_`–`144_`) : `VOICE_IDENTITY_CATALOG_REMOTE_MIGRATION_APPLIED_EMPTY_RUNTIME_OFF`.

Le chemin est câblé. Catalog remote appliqué : trois tables **vides**, alignement **31/31**. Config locale Voice prête (4 prefixes distincts, 0 ID exposé). `ELEVENLABS_VOICE_ID` historique = Mei, plus un fallback. **0** ElevenLabs. Le projet I2V n’a pas encore de narrateur choisi.
...
Préparer seed/consentements en lecture seule. **Aucune** écriture Production. **Aucun** appel ElevenLabs. **Aucun** lipsync.
...
Si `144_` est STOP, la porte suivante est le preflight seed/consent (sans write). Aucun second submit I2V.
