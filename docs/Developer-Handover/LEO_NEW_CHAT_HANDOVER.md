...
Voice/TTS `/director` est **WIRED_DISABLED** (`140_`/`141_`) : `BLOCKED_VOICE_NARRATOR_BINDING_CONFIG_UNAVAILABLE`.

Le chemin est câblé et testé en fakes. **0** ElevenLabs. Le projet I2V a un segment `voice_over` canonique (script `349e2792…` / `segment-2` / 81 car. / hash `f228654f…`). La voix configurée (`env:ELEVENLABS_VOICE_ID`) collisionne avec l’identité **Mei**. Le binding narrateur n’est **pas** persisté. L’Auth `141_` n’autorise pas de substituer Mei. Le Privacy Pack MV-001 ≠ consentement Voice. Le pricing catalogue n’est pas ferme.
...
Décider une voix narrateur **distincte** de Tom/Mei, **ou** autoriser explicitement Mei comme narrateur `voice_over` de ce projet. **Aucun** appel ElevenLabs. **Aucun** lipsync.

## Première action du nouveau chat

1. Lire entièrement `CURRENT_STATE_AND_RESUME.md`.
2. Lire ce fichier.
3. Recevoir le prochain rapport STOP de Cursor concernant `AUTH_11C_VOICE_NARRATOR_IDENTITY_DECISION`.
4. Ne pas refaire les phases déjà terminées.
5. Vérifier le rapport et préparer la prochaine porte.

Si `141_` est STOP, la porte corrective est la décision d’identité narrateur. Aucun live preflight Voice tant que cette porte n’est pas passée. Aucun second submit I2V.
