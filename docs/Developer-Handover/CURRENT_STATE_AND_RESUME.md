...
verifiedAt=2026-08-27T13:20:00+02:00
documentedHead=0faf2d9
headStatus=pending commit
lastPhaseReport=165_PHASE_RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT.md
nextPhase=AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER
budgetHard=437
budgetCommitted=391
budgetReserved=0
budgetAvailable=46
...
**Statut global :** `RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT_READY`  
**Dernière vérification :** 2026-08-27 · bind preflight RideCloud `165_`  
**Auteur de la mise à jour :** Cursor · `AUTH_RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT_NO_PROVIDER`  
**Branche :** `main`  
**HEAD local au départ de `165_` :** `0faf2d9` · create idempotent **`ec57ed5`**  
**origin/main au départ :** `0faf2d9` · ahead/behind **0/0**  
...
**Dernier rapport fonctionnel :** [`165_PHASE_RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT.md`](./165_PHASE_RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT.md)  
...
**Prochaine phase exacte :** `AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER`
...
| RideCloud promo | **BIND PREFLIGHT READY** · draft + brief rev.1 | `165_` · contrat `881760c3…` · 0 write |
| Prochaine étape | **PREPARED** | `AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER` · 0 provider · 0¢ |
...
| Supabase / migrations | PASS_REAL | MT-005 `82_` + Voice catalog `144_` + grants `147_` + seed `149_` + binding `151_` | **32/32** | — | 0 | 1 binding female · execution=false | first paid TTS |
...
| Migrations Production | **32** |
| Migrations locales | **32** fichiers `studio/supabase/migrations/` |
| Alignement | **32/32** |
...
|---|---|---|---|
| Unitaires | **1892/1892** | suite + bind preflight RideCloud | 2026-08-27 `165_` |
| Typecheck / lint / build | **PASS** | first paid TTS + docs | `153_` / `157_` |
| migrations-static | PASS | **32/32** | `153_` |
| Secret scan | **PASS** | module + docs · `VOICE_IDS_EXPOSED=false` | `156_` |
| Tests ciblés | bind preflight READY · refus locators / doublon | cette phase | `165_` |
...
| Fraîcheur living handover | PASS | tooling | `165_` |
...
**`AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER`** — preflight local de l’extension de schéma pour `storyboard_contract` + `media_input_manifest`. **Aucun** apply distant, provider, média, persist bind, dépense, run ou flag. **Non exécutée ici.**
...
```text
Contexte : VHS V2 · 11A/11B/11C CLOSED · RideCloud bind preflight READY · budget 437/391/0/46 ¢
Dernier verdict : RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT_READY (165_)
Preuve : kinds storyboard_contract / media_input_manifest · 0 persist · CHECK actuel refuse
Blocage : kinds absents du schéma · pas de génération
Mission : uniquement AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER
Auth consommées : bind preflight (165_) · create idempotent (164_) · create preflight (163_) · polish VO (162_)
Interdit : provider · dépense · média Git · lecture/upload pack · apply schéma · TTS · export
Attendu : preflight schéma kinds seulement
```
...
Lis ensuite CURRENT_STATE_AND_RESUME.md et 165_PHASE_RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT.md.
Vérifie Git. RideCloud bind preflight READY. Pack 158_+159_. Storyboard 26 s. 11C CLOSED. Budget 437/391/0/46.
Flags considérés OFF. Auth 165_ / 164_ / 163_ / 162_ / 161_ / 160_ / 159_ / 158_ / 157_ / 156_ / 155_ / 153_ consommées. maySubmit=false.
Une autorisation provider d’un chat précédent n’est JAMAIS réutilisable.
Aucun provider. 0¢. Aucun persist bind. Aucun média Git. Aucun lipsync.
Prochaine porte : AUTH_RIDECLOUD_SEPARATE_PROJECT_BIND_KIND_SCHEMA_PREFLIGHT_NO_PROVIDER. 0 provider. 0¢.
...
|---|---|---|---|---|---|---|
| 2026-08-27 | `165_` RideCloud bind preflight | RIDECLOUD_SEPARATE_PROJECT_STORYBOARD_PACK_BIND_PREFLIGHT_READY | pending | 0 | **docs + contrat** · 0 persist | schema kinds |
