...
export const PRODUCTS_ROOT = path.join(REPO_ROOT, "products");
...
export interface Product {
  ...
  color?: string;
  url?: string;
  screens: string[]; // file names under products/<id>/screens/
}

export function listProducts(): Product[] {
  const out: Product[] = [];
  for (const id of listDirs(PRODUCTS_ROOT)) {
    const metaRaw = readText(path.join(PRODUCTS_ROOT, id, "product.json"));
    let meta: Record<string, unknown> = {};
    if (metaRaw) {
      try {
    ...
    out.push({
      id,
      name: (meta.name as string) ?? id,
      pitch: meta.pitch as string | undefined,
      color: meta.color as string | undefined,
      url: meta.url as string | undefined,
      screens: listImageFiles(path.join(PRODUCTS_ROOT, id, "screens")),
    });
  }
...
export function saveProduct(input: {
  ...
  return { id, ...meta, screens: listImageFiles(path.join(dir, "screens")) };
}

export function addProductScreens(productId: string, dataUrls: string[]): string[] {
  const dir = path.resolve(PRODUCTS_ROOT, safeId(productId), "screens");
  if (!dir.startsWith(PRODUCTS_ROOT + path.sep)) throw new Error("Invalid product");
  fs.mkdirSync(dir, { recursive: true });
  const saved: string[] = [];
  dataUrls.forEach((du, i) => {
    const m = /^data:image\/(png|jpe?g|webp);base64,(.+)$/i.exec(du);
    if (!m) return;
...
export function readProductScreen(productId: string, name: string): { buffer: Buffer; mime: string } | null {
  const dir = path.resolve(PRODUCTS_ROOT, safeId(productId), "screens");
  if (!dir.startsWith(PRODUCTS_ROOT + path.sep)) return null;
