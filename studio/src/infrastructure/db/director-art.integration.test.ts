/**
 * Director art persistence against LOCAL Supabase (VHS-120B).
 */
...
if (!gate.ok) throw new Error(`VHS-120B: ${gate.reason}`);
...
async function seedWs() {
  ...
  await client.from("workspaces").insert({ id: workspaceId, slug: `s120-${workspaceId.slice(0, 8)}`, name: "Art 120B", mode: "single_workspace" });
  await client.from("workspace_budget_policies").insert({ workspace_id: workspaceId, hard_limit_minor: 100_000, currency: "USD" });
  return workspaceId;
}

test("VHS-120B — chain through visual_direction", async () => {
  ...
  const brief = finalizeBrief({
    ...
    fields: {
      projectName: "Campagne 120B", subjectType: "product", subjectName: "Widget",
  ...
  }, { id: artifactId, projectId, createdBy: "tester", correlationId: "corr-120b-it", createdAt: "2026-08-03T14:00:00.000Z", revision: 1 });
  ...
  await createSupabaseCreateProjectWithBriefPort({ client }).execute({
    ...
    correlationId: "corr-120b-it", actorType: "shared_password", actorId: "tester", createdBy: "tester",
  });

  let scriptSegmentIds: string[] = [];
  const env = {
    ...
    DIRECTOR_V2_PAID_AI_ENABLED: "1", OPENAI_API_KEY: "sk-test-local",
  ...
  const marketing = await stack.analyzeMarketing.execute({ projectId, expectedBriefRevision: 1 }, { correlationId: "corr-120b-marketing", mode: "execute" });
  assert.equal(marketing.status, "completed", JSON.stringify(marketing));
  assert.equal((await stack.analyzeCreative.execute({ projectId, expectedMarketingPlanRevision: 1 }, { correlationId: "corr-120b-creative", mode: "execute" })).status, "completed");
  assert.equal((await stack.writeScript.execute({ projectId, expectedCreativeConceptRevision: 1 }, { correlationId: "corr-120b-script", mode: "execute" })).status, "completed");

  const dry = await stack.analyzeArt.dryRun({ projectId }, { correlationId: "corr-120b-art-dry", mode: "dry-run" });
  assert.equal(dry.providerCalled, false);
  assert.equal(dry.executable, true);

  const exec = await stack.analyzeArt.execute({ projectId, expectedVideoScriptRevision: 1 }, { correlationId: "corr-120b-art-exec", mode: "execute" });
  assert.equal(exec.status, "completed", JSON.stringify(exec));
  if (exec.status !== "completed") return;
  assert.equal(exec.visualDirection.status, "ready");
  assert.ok(exec.visualDirection.palette?.length);

  const replay = await stack.analyzeArt.execute({ projectId, expectedVideoScriptRevision: 1 }, { correlationId: "corr-120b-art-replay", mode: "execute" });
  assert.equal(replay.status, "existing");

  const { count } = await client.from("project_artifacts").select("*", { count: "exact", head: true })
    .eq("project_id", projectId).eq("artifact_type", "visual_direction");
  assert.equal(count, 1);

  const { data: audit } = await client.from("audit_log").select("action")
    .eq("project_id", projectId).eq("action", "director.art.completed");
