import type { CreativeDirectorRunPort } from "@/application/directors/creative/analyze-for-project";
import type { Json } from "../database.types";
import { mapSupabaseError, PersistenceError } from "../errors";
import type { V2DbClient } from "../supabase-server";

export function createSupabaseCreativeDirectorRunPort(deps: {
  client: V2DbClient;
  workspaceId: string;
}): CreativeDirectorRunPort {
  const { client, workspaceId } = deps;
  return {
    async beginOrGet(input) {
      const { data, error } = await client.rpc(
        "begin_or_get_creative_director_run" as never,
        {
          p_id: input.id,
      ...
      if (!row?.director_run_id)
        throw new PersistenceError("unknown", "Creative run incomplete.");
    ...
    async persistConcept(input) {
      const { data, error } = await client.rpc(
        "persist_creative_concept" as never,
        {
          p_workspace_id: input.workspaceId,
          p_project_id: input.projectId,
          p_director_run_id: input.directorRunId,
          ...
          p_brief_artifact_id: input.briefArtifactId,
          p_brief_revision: input.briefRevision,
          p_concept: input.concept as Json,
          p_schema_version: input.schemaVersion,
          p_correlation_id: input.correlationId,
      ...
      if (!row?.artifact_id || row.revision == null)
        throw new PersistenceError("unknown", "Persist creative concept incomplete.");
      return {
    ...
    async loadActiveCreativeConcept(projectId) {
      ...
      return art ? { revision: art.revision as number, value: art.value } : null;
    },
  };
}
