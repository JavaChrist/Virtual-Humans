...
const BodySchema = z.discriminatedUnion("mode", [
  ...
  z.object({
    ...
    expectedVisualDirectionRevision: z.number().int().positive(),
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, { route: "/api/director/projects/[projectId]/storyboard", operation: "director.storyboard.get" });
  ...
  try {
    const dryRun = await createDirectorPersistenceStack().analyzeStoryboard.dryRun({ projectId }, { correlationId: obs.context.correlationId, mode: "dry-run" });
    return obs.json({ dryRun, storyboard: dryRun.existingStoryboard ?? null });
  } catch (error) {
    ...
    return obs.json({ error: "Lecture storyboard impossible." }, { status: 500 });
  }
}

export async function POST(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, { route: "/api/director/projects/[projectId]/storyboard", operation: "director.storyboard.post" });
  if (!canUseDirectorV2Persistence()) return obs.json({ error: "Persistance Director désactivée." }, { status: 404 });
  const { projectId } = await params;
  if (!isUuid(projectId)) return obs.json({ error: "Identifiant invalide." }, { status: 400 });
  let raw: unknown;
  try { raw = await req.json(); } catch { return obs.json({ error: "JSON invalide." }, { status: 400 }); }
  ...
  try {
    const analyzer = createDirectorPersistenceStack().analyzeStoryboard;
    if (body.data.mode !== "execute") {
      const dryRun = await analyzer.dryRun({ projectId }, { correlationId, mode: "dry-run" });
    ...
    const result = await analyzer.execute({
      ...
      expectedCreativeConceptRevision: body.data.expectedCreativeConceptRevision,
      expectedMarketingPlanRevision: body.data.expectedMarketingPlanRevision,
    ...
    if (error instanceof V2SupabaseConfigError) return obs.json({ error: error.message }, { status: 503 });
    logger.error("route.failure", obs.context, error);
    return obs.json({ status: "failed", error: { code: "internal_error", retryable: false, message: "Échec du storyboard." } }, { status: 500 });
  }
}
