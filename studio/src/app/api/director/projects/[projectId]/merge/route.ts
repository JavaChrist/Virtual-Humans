...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/merge",
    operation: "director.merge.get",
  });
  ...
  try {
    ...
    const prepareDry = await stack.prepareMerge.dryRun(
      { projectId },
      { correlationId: obs.context.correlationId, mode: "dry-run" },
    );
    const executeDry = await stack.executeMerge.dryRun(
      { projectId },
      { correlationId: obs.context.correlationId, mode: "dry-run" },
    );
    const mp = await stack.artifacts.getActive(projectId, "merge_plan");
    let mergeOutcome = null as unknown;
    if (mp) {
    ...
    return obs.json({ error: "Lecture merge impossible." }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    const stack = createDirectorPersistenceStack();
    if (body.data.mode === "dry_run" || body.data.mode === "dry-run") {
      const prepareDryRun = await stack.prepareMerge.dryRun(
        { projectId },
        { correlationId, mode: "dry-run" },
      );
      const executeDryRun = await stack.executeMerge.dryRun(
        { projectId },
        { correlationId, mode: "dry-run" },
      );
    ...
    return obs.json(
      {
        status: "failed",
        error: { code: "internal_error", retryable: false, message: "Échec du merge." },
      },
      { status: 500 },
    );
  }
