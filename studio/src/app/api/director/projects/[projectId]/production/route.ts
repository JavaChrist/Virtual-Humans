...
const BodySchema = z.discriminatedUnion("mode", [
  ...
  z.object({
    ...
    expectedGenerationPlanRevision: z.number().int().positive(),
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/production",
    operation: "director.production.get",
  });
  ...
  try {
    const dryRun = await createDirectorPersistenceStack().startProduction.dryRun(
      { projectId },
      { correlationId: obs.context.correlationId, mode: "dry-run" },
    );
    return obs.json({ dryRun, run: dryRun.existingRun ?? null });
  } catch (error) {
    ...
    return obs.json({ error: "Lecture production impossible." }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/production",
    operation: "director.production.post",
  });
  if (!canUseDirectorV2Persistence()) {
    return obs.json({ error: "Persistance Director désactivée." }, { status: 404 });
  }
  ...
  try {
    const start = createDirectorPersistenceStack().startProduction;
    if (body.data.mode !== "execute") {
      const dryRun = await start.dryRun({ projectId }, { correlationId, mode: "dry-run" });
      return obs.json({ dryRun });
    }
    const denied = directorActionHttp(
      authorizeDirectorAction({ routeId: "production", method: "POST", mode: "execute" }),
    );
    if (denied) {
      return obs.json(
        {
    ...
    const result = await start.execute(
      {
        projectId,
        expectedGenerationPlanRevision: body.data.expectedGenerationPlanRevision,
        confirmation: true,
      },
    ...
    return obs.json(
      {
        ...
        error: {
          code: "internal_error",
          retryable: false,
          message: "Échec du démarrage production.",
        },
      },
      { status: 500 },
    );
