...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/quality",
    operation: "director.quality.get",
  });
  ...
  try {
    ...
    const dryRun = await stack.evaluateQuality.dryRun(
      { projectId },
      { correlationId: obs.context.correlationId, mode: "dry-run" },
    );
    const qr = await stack.artifacts.getActive(projectId, "quality_report");
    let quality = dryRun.quality ?? null;
    if (qr) {
      const loaded = await stack.artifacts.load(qr.artifactId);
      if (loaded) quality = loaded.value as typeof quality;
    }
    const pr = await stack.artifacts.getActive(projectId, "production_result");
    let delivery = null as unknown;
    if (pr) {
      const loaded = await stack.artifacts.load(pr.artifactId);
      const value = loaded?.value as { delivery?: unknown; status?: string } | undefined;
      delivery = value?.delivery ?? null;
    }
    return obs.json({
      dryRun,
      quality,
      qualityReportRevision: qr?.revision ?? null,
      productionResultRevision: pr?.revision ?? dryRun.productionResultRevision,
      delivery,
    });
  } catch (error) {
    ...
    return obs.json({ error: "Lecture qualité impossible." }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/quality",
    operation: "director.quality.post",
  });
  if (!canUseDirectorV2Persistence()) {
    return obs.json({ error: "Persistance Director désactivée." }, { status: 404 });
  }
  ...
  try {
    const evaluate = createDirectorPersistenceStack().evaluateQuality;
    if (body.data.mode !== "execute") {
      const dryRun = await evaluate.dryRun({ projectId }, { correlationId, mode: "dry-run" });
      return obs.json({ dryRun });
    }
    const denied = directorActionHttp(
      authorizeDirectorAction({ routeId: "quality", method: "POST", mode: "execute" }),
    );
    if (denied) {
      return obs.json(
        {
    ...
    const result = await evaluate.execute(
      {
        projectId,
        confirmation: true,
        productionRunId: body.data.productionRunId,
        allowPartial: body.data.allowPartial,
      },
    ...
    return obs.json(
      {
        status: "failed",
        error: { code: "internal_error", retryable: false, message: "Échec du contrôle qualité." },
      },
      { status: 500 },
    );
  }
