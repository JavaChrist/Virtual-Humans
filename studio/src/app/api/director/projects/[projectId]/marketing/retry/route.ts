/**
 * POST — Explicit human Marketing retry (VHS-128).
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, {
    route: "/api/director/projects/[projectId]/marketing/retry",
    operation: "director.marketing.retry",
  });
  ...
  try {
    ...
    logger.info("director.marketing.retry.started", obs.context, {
      projectId,
      previousRunId: parsed.data.previousRunId,
    });

    const result = await stack.analyzeMarketing.executeRetry(
      {
        projectId,
        previousRunId: parsed.data.previousRunId,
    ...
    if (result.status === "failed") {
      if (isCanonicalFailureCode(result.code)) {
        ...
        logger.info("director.marketing.retry.failed", obs.context, {
          projectId,
          directorRunId: result.directorRunId,
    ...
    logger.info("director.marketing.retry.completed", obs.context, {
      projectId,
      status: result.status,
      directorRunId: result.directorRunId,
    });
