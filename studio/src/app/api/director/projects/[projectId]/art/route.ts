...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, { route: "/api/director/projects/[projectId]/art", operation: "director.art.get" });
  ...
  try {
    const dryRun = await createDirectorPersistenceStack().analyzeArt.dryRun({ projectId }, { correlationId: obs.context.correlationId, mode: "dry-run" });
    return obs.json({ dryRun, visualDirection: dryRun.existingVisualDirection ?? null });
  } catch (error) {
    ...
    return obs.json({ error: "Lecture direction art impossible." }, { status: 500 });
  }
}

export async function POST(req: NextRequest, { params }: RouteParams) {
  const obs = startObservedRoute(req, { route: "/api/director/projects/[projectId]/art", operation: "director.art.post" });
  if (!canUseDirectorV2Persistence()) return obs.json({ error: "Persistance Director désactivée." }, { status: 404 });
  const { projectId } = await params;
  if (!isUuid(projectId)) return obs.json({ error: "Identifiant invalide." }, { status: 400 });
  let raw: unknown;
  ...
  try {
    const analyzer = createDirectorPersistenceStack().analyzeArt;
