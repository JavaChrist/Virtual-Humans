...
export function createWriteScriptForProject(deps:WriteScriptForProjectDeps):WriteScriptForProject {
  ...
  async function dry(input:ScriptProjectInput):Promise<ScriptProjectDryRunResult> {
    ...
    return {executable:ai.executable,providerCalled:false,executionAvailable:ai.executable&&canExecuteScriptAi(env)&&ai.pricingConfigured,briefRevision:brief.revision,briefArtifactId:brief.artifactId,marketingPlanRevision:plan.revision,marketingPlanArtifactId:plan.artifactId,creativeConceptRevision:concept.revision,creativeConceptArtifactId:concept.artifactId,model:ai.model,promptVersion:ai.promptVersion,schemaVersion:ai.schemaVersion,timingEngineVersion:SPEECH_TIMING_ENGINE_VERSION,pricingConfigured:ai.pricingConfigured,estimatedCostMinor:estimated,currency:price?.currency,validations:ai.validations,warnings:ai.warnings,missingInformation:ai.validations.filter(v=>!v.passed).map(v=>({code:v.code,message:v.message})),targetDuration:brief.value.durationSeconds,existingScript:existing?stored(existing.value,existing.revision):undefined};
  }
  return { dryRun: async(input)=>dry(input), async execute(input,context) {
    ...
    if(!canExecuteScriptAi(env))return failed("script_ai_disabled","Rédaction Script IA désactivée.",503);
    let config;try{config=parseOpenAIScriptConfig(env)}catch{return failed("invalid_config","Configuration Script IA invalide.",503)}
