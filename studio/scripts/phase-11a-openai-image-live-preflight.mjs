#!/usr/bin/env node
/**
 * Phase 11A-PREFLIGHT-LIVE-NO-PROVIDER
 *
 *   CONFIRM_PHASE_11A_OPENAI_IMAGE_PREFLIGHT_NO_PROVIDER=1 \
 *   node --import tsx scripts/phase-11a-openai-image-live-preflight.mjs
 *
 * Opens Director/Persistence/Paid + VHS124 OpenAI image exception (worker OFF),
 * redeploys ON from be415f5, runs HTTP prompts dry-run + local allowlist dry-run
 * (providerCalled=false), then finally closes all flags and redeploys OFF.
 *
...
const EXPECTED_COMMIT = "be415f5401eac0720d2a525201d53a0c63f4e644";
const EXPECTED_COMMIT_SHORT = "be415f5";
...
const AUTH = "AUTH_PHASE_11A_OPENAI_IMAGE_PREFLIGHT_NO_PROVIDER";
const SALT = `11a-openai-image-preflight-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${randomBytes(4).toString("hex")}`;
...
async function captureCounters(db) {
  ...
  const countOrZero = (r) => {
    if (r.error) {
      if (
        /does not exist|Could not find|schema cache/i.test(r.error.message || "")
      ) {
        return { missing: true, count: null, error: r.error.message };
      }
      fail(`counter query failed: ${r.error.message}`);
    }
  ...
  return {
    ...
    scene_package_set_active: countOrZero(packages),
    generation_plan_active: countOrZero(plans),
    human_review_decisions: countOrZero(reviews),
    schema_migrations: countOrZero(migrations),
...
async function main() {
  if (process.env.CONFIRM_PHASE_11A_OPENAI_IMAGE_PREFLIGHT_NO_PROVIDER !== "1") {
    console.error(
      "Refused: set CONFIRM_PHASE_11A_OPENAI_IMAGE_PREFLIGHT_NO_PROVIDER=1",
    );
    process.exit(2);
  }
  ...
  try {
    ...
    if ((head.stdout || "").trim() !== EXPECTED_COMMIT) {
      // Allow dirty docs tree? User said source runtime be415f5 — deploy must be be415f5.
      console.log(
        `WARN local HEAD=${(head.stdout || "").trim()} (runtime deploy must still be ${EXPECTED_COMMIT_SHORT})`,
      );
    }
...
function writeReport(report) {
  ...
  const path = join(outDir, "phase-11a-openai-image-live-preflight-report.json");
