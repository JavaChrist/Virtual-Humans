/**
 * Phase 10E-PREP — verify MarketingPlan + CreativeConcept + VideoScript (read-only).
 * Confirms no active visual_direction yet. Never prints artifact bodies.
 *
 * Requires: CONFIRM_PHASE_10E_REMOTE_READ=1
 */
...
if (process.env.CONFIRM_PHASE_10E_REMOTE_READ !== "1") {
  console.error(
    "Refused: set CONFIRM_PHASE_10E_REMOTE_READ=1 (reads Production via .env.remote.local)."
  );
  process.exit(2);
}
...
const report = {
  ...
  existingVisualDirection: activeArt
    ? {
        artifactId: activeArt.artifact_id,
        revision: activeArt.revision,
      }
    : null,
  reusable:
    mktZod.success &&
...
const outPath = resolve(
  evidenceDir,
  `phase-10e-prep-upstream-${Date.now()}.json`
);
