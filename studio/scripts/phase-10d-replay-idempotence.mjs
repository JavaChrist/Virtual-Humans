/**
 * Phase 10D — idempotent replay of Script execute (must not call provider).
 * Refused during PREP.
 */
...
const projectId =
  process.argv[2] ||
  process.env.PHASE_10D_PROJECT_ID ||
  "984507af-a89e-4644-8ea3-344797baa974";
const expectedCreativeConceptRevision = Number(
  process.env.PHASE_10D_EXPECTED_CREATIVE_REVISION || "1"
);
const expectedMarketingPlanRevision = Number(
  process.env.PHASE_10D_EXPECTED_MARKETING_REVISION || "1"
);
...
const tmp = resolve(studioRoot, ".env.vercel.10d.replay.tmp");
...
const t0 = Date.now();
const res = await fetch(`${BASE}/api/director/projects/${projectId}/script`, {
  method: "POST",
  headers: {
    ...
    "x-correlation-id": "corr-10d-replay-idem",
  },
  body: JSON.stringify({
    mode: "execute",
