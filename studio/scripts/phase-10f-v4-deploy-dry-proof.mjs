/**
 * Phase 10F-V4-DEPLOY-PREFLIGHT — live dry-run gates (NO execute).
 * Exits 0 only when v4 prompt/mandatory continuity/schema/salt/budget gates match.
 */
...
const SALT =
  ...
  "10f-storyboard-v4-20260811";
const EXPECT_V4_FP = "801c34a1080bbcf0";
...
const EXPECT_SALT_FP = "05be5ef9a08d005f";
...
if (SALT !== "10f-storyboard-v4-20260811" || saltFp !== EXPECT_SALT_FP) {
  console.error("FAIL: unexpected salt identity (value not printed)");
  process.exit(2);
}

const tmp = resolve(studioRoot, ".env.vercel.10f.v4.tmp");
...
const corr = `corr-10f-v4-${Date.now()}-dry`;
...
const baseV4 = { ...baseV2, promptVersion: "storyboard-analyzer-v4" };
...
const v3Fp = fp(
  storyboardIdempotencyFields({
    ...baseV3,
    idempotencySalt: "10f-storyboard-v3-20260810",
  }).key,
);
const v4Fp = fp(
  storyboardIdempotencyFields({
...
const slotCount =
  dry.mandatoryContinuityTokenCount ?? dry.requiredContinuityTokenCount;
const uniqueCount = dry.mandatoryContinuityUniqueTokenCount;
const scopeCount =
  dry.mandatoryContinuityScopeCount ?? dry.requiredContinuityScopeCount;
const coverage =
  dry.mandatoryContinuityCoverage ?? dry.requiredContinuityCoverage;
const tokensFp =
  dry.mandatoryContinuityTokensFingerprint ??
  dry.requiredContinuityTokensFingerprint;
const anyOfOk =
...
const checks = {
  ...
  prompt: dry.promptVersion === "storyboard-analyzer-v4",
  refuseV2: dry.promptVersion !== "storyboard-analyzer-v2",
  refuseV3: dry.promptVersion !== "storyboard-analyzer-v3",
  schema: dry.schemaVersion === "1.0.0",
  idempotencyKeyVersion:
    dry.idempotencyKeyVersion === "storyboard-analyzer-v4:1.0.0",
  continuitySemantics:
    dry.continuitySemantics === "mandatory-projected-tokens" ||
    dry.continuitySemantics == null, // tolerate if older field absent but counts pass
  ...
  slots24: slotCount === 24,
  unique9: uniqueCount === 9,
  scopes5: scopeCount === 5,
  coverageComplete: coverage === "complete",
  tokensFingerprint: tokensFp === EXPECT_TOKENS_FP,
...
const evidence = {
  ...
  dryRun: {
    ...
    continuitySemantics: dry.continuitySemantics ?? null,
    ...
    mandatoryContinuityTokenCount: slotCount ?? null,
    mandatoryContinuityUniqueTokenCount: uniqueCount ?? null,
    mandatoryContinuityScopeCount: scopeCount ?? null,
    mandatoryContinuityCoverage: coverage ?? null,
    mandatoryContinuityTokensFingerprint: tokensFp ?? null,
...
const path = resolve(studioRoot, ".tmp/phase-10f-v4-deploy-dry.json");
