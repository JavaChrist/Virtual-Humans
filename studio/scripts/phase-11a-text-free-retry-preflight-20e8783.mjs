 ...
 *   node --import tsx scripts/phase-11a-text-free-retry-preflight-20e8783.mjs
 *
 * Validates exact commit 20e8783, overlay no-text policy, synthetic compositor,
...
const EXPECTED_COMMIT_SHORT = "20e8783";
...
const SALT = `11a-text-free-retry-preflight-20e8783-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${randomBytes(4).toString("hex")}`;
...
async function proveOverlayPipeline({ scenePkg, scriptValue, promptMod, promptGate }) {
  ...
  return {
    verdict: promptContainsMarketingCopy ? "BLOCKED_TEXT_LEAK_TO_PROVIDER_PROMPT" : null,
...
async function localAllowlistDryRun(envForException) {
  ...
  const built = (() => {
    try {
      ...
      const msg = String(e.message || e);
      if (/overlay copy|screenText copy leaked|screenText copy must not appear|draw words/i.test(msg)) {
        overlayProof.verdict = "BLOCKED_TEXT_LEAK_TO_PROVIDER_PROMPT";
...
async function main() {
  ...
  if (process.env.PHASE_11A_LOCAL_ONLY === "1") {
    const reportPath = join(studioRoot, ".tmp", "phase-11a-text-free-retry-preflight-20e8783-report.json");
  ...
  try {
    ...
    if ((dirty.stdout || "").trim()) {
      ...
      const lines = (dirty.stdout || "")
        ...
        .filter(
          (l) =>
            !l.includes("phase-11a-text-free-retry-preflight-20e8783.mjs") &&
            !l.includes(".tmp/") &&
            !l.includes("112_PHASE_11A_TEXT_FREE_IMAGE_RETRY_PREFLIGHT.md"),
        );
      if (lines.length > 0) {
...
function writeReport(report) {
  ...
  const path = join(outDir, "phase-11a-text-free-retry-preflight-20e8783-report.json");
