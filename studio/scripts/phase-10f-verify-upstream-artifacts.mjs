/**
 * Phase 10F-PREP — verify Marketing + Creative + Script + VisualDirection (read-only).
 * Confirms no active storyboard_project. Never prints artifact bodies.
 *
...
if (process.env.CONFIRM_PHASE_10F_REMOTE_READ !== "1") {
  console.error(
    "Refused: set CONFIRM_PHASE_10F_REMOTE_READ=1 (reads Production via .env.remote.local)."
  );
  process.exit(2);
}
...
const scr = await loadArt(SCRIPT_ARTIFACT_ID);
const vis = await loadArt(VISUAL_ARTIFACT_ID);
...
const activeArt = await loadActive("visual_direction");
...
const scrZod = VideoScriptSchema.safeParse(scr.value);
const visZod = VisualDirectionSchema.safeParse(vis.value);

const scriptSegCount = scrZod.success ? scrZod.data.segments.length : null;
const visualSegCount = visZod.success ? visZod.data.segments.length : null;
...
const report = {
  ...
  videoScript: {
    artifactId: scr.id,
    projectIdMatch: scr.project_id === PROJECT_ID,
    revision: scr.revision,
    zodOk: scrZod.success,
    valueSha256Prefix: hashValue(scr.value),
    correlationId: scr.correlation_id,
    createdBy: scr.created_by,
    activeMatches:
      Boolean(activeScript) && activeScript.artifact_id === SCRIPT_ARTIFACT_ID,
    activeRevision: activeScript?.revision ?? null,
    segmentCount: scriptSegCount,
  },
  visualDirection: {
    artifactId: vis.id,
    projectIdMatch: vis.project_id === PROJECT_ID,
    revision: vis.revision,
    zodOk: visZod.success,
    valueSha256Prefix: hashValue(vis.value),
    correlationId: vis.correlation_id,
    createdBy: vis.created_by,
    activeMatches:
      Boolean(activeArt) && activeArt.artifact_id === VISUAL_ARTIFACT_ID,
    activeRevision: activeArt?.revision ?? null,
    segmentCount: visualSegCount,
    promptVersionHint:
      visZod.success && typeof vis.value === "object" && vis.value
        ? (vis.value.promptVersion ??
          vis.value.rationale?.promptVersion ??
          null)
        : null,
  },
  existingStoryboard: activeStoryboard
    ? {
        artifactId: activeStoryboard.artifact_id,
        revision: activeStoryboard.revision,
      }
    : null,
  segmentAlignment:
    scriptSegCount != null &&
    visualSegCount != null &&
    scriptSegCount === visualSegCount &&
    scriptSegCount === 5,
  reusable:
    mktZod.success &&
    creZod.success &&
...
const outPath = resolve(
  evidenceDir,
  `phase-10f-prep-upstream-${Date.now()}.json`
);
