#!/usr/bin/env node
/**
 * Phase 11A-PREFLIGHT-FINAL-AFTER-PROMPT-GATE-FIX
 *
 *   CONFIRM_PHASE_11A_FINAL_PREFLIGHT_9952380=1 \
 *   node --import tsx scripts/phase-11a-final-preflight-9952380.mjs
 *
 * Deploys/validates exact commit 9952380 (prompt-gate [DATA:…] fix),
...
const EXPECTED_COMMIT = "995238091a92d5b80b5a1327c783747d86f5f2ae";
const EXPECTED_COMMIT_SHORT = "9952380";
...
const AUTH = "AUTH_PHASE_11A_FINAL_PREFLIGHT_COMMIT_9952380_NO_PROVIDER";
const SALT = `11a-final-preflight-9952380-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${randomBytes(4).toString("hex")}`;
...
async function main() {
  if (process.env.CONFIRM_PHASE_11A_FINAL_PREFLIGHT_9952380 !== "1") {
    console.error(
      "Refused: set CONFIRM_PHASE_11A_FINAL_PREFLIGHT_9952380=1",
    );
    process.exit(2);
  }
  ...
  try {
    ...
    if ((dirty.stdout || "").trim()) {
      ...
      const lines = (dirty.stdout || "")
        ...
        .filter(
          (l) =>
            !l.includes("phase-11a-final-preflight-9952380.mjs") &&
            !l.includes(".tmp/"),
        );
      if (lines.length > 0) {
...
function writeReport(report) {
  ...
  const path = join(outDir, "phase-11a-final-preflight-9952380-report.json");
