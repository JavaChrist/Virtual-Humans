#!/usr/bin/env node
/**
 * Phase 11A existing-provider PNG composition LIVE preflight.
 *
 *   CONFIRM_PHASE_11A_EXISTING_PROVIDER_COMPOSITION_PREFLIGHT=1 \
 *   node --import tsx scripts/phase-11a-existing-provider-composition-preflight-60cc335.mjs
 *
 * Reads the existing provider PNG once via a 60s signed URL, decodes filters
 * 0–4, composes the exact French overlay in memory, then closes all gates.
 *
...
const KNOWN_READY_HOST =
  "virtual-humans-diorj5zdf-javachrist-projects.vercel.app";
...
const AUTH = "AUTH_11A_COMPOSE_EXISTING_PROVIDER_PNG_FILTERS";
...
const SALT = `11a-compose-existing-provider-preflight-60cc335-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${randomBytes(4).toString("hex")}`;
...
async function assertHistoricalTargets(db) {
  ...
  if (rejectedReviews.length !== 1) fail(`expected exactly 1 REJECT got ${rejectedReviews.length}`);
  ...
  if (composedChildren.length !== 0) fail("composed child already exists");
  ...
  if (!run) fail("pending run missing");
  if (String(run.status) !== "running") fail(`run status ${run.status}`);
...
function writeReport(report) {
  ...
  writeFileSync(join(dir, "phase-11a-existing-provider-composition-preflight.json"), `${blob}\n`);
}
...
async function main() {
  planMod.assertPhase11ACompositionPreflightConfirm(process.env);
  if (process.argv.includes("--execute") && process.env.PHASE_11A_ALLOW_EXECUTE === "1") {
    fail("PHASE_11A_ALLOW_EXECUTE is forbidden");
  }
  ...
  try {
    ...
    if (composed.compositorVersion !== "phase-11a-bitmap-compositor-1.0.0") {
      fail("compositor version");
    }
    ...
    if (targetsAfter.runStatus !== "running" || targetsAfter.waitingReason !== "needs_review") {
      fail("run mutated");
    }
    report.deltasZero = true;
    report.verdict = "READY_FOR_EXISTING_PROVIDER_ASSET_COMPOSITION_EXECUTION";
  } catch (e) {
    const msg = String(e.message || e);
  ...
  if (report.verdict !== "READY_FOR_EXISTING_PROVIDER_ASSET_COMPOSITION_EXECUTION") {
    process.exitCode = 2;
  }
}
