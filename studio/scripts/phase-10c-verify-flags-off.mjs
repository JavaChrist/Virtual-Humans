/**
 * Phase 10C — confirm Production Director persistence / AI path is OFF after close-out.
 * Read-only: vercel env pull + GET /api/director/projects (expect 404).
 */
...
const tmp = resolve(studioRoot, ".env.vercel.10c.off.tmp");
  ...
  JSON.stringify(
    {
      ...
      CURRENT_RUNTIME_REAL_AI:
        res.status === 404 && allOff ? "OFF" : "UNKNOWN_OR_STILL_ON",
    },
    null,
    2
  )
);
process.exit(res.status === 404 && allOff ? 0 : 2);
