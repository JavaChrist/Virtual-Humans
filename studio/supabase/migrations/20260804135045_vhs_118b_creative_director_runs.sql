-- VHS-118B — persisted Creative Director runs. Local additive migration only.
BEGIN;
...
ALTER TABLE public.director_runs ADD CONSTRAINT director_runs_type_check
  CHECK (director_type IN ('marketing', 'creative'));
...
ALTER TABLE public.director_runs ADD CONSTRAINT director_runs_input_type_check
  CHECK (input_artifact_type IN ('video_project_brief', 'marketing_plan'));

CREATE OR REPLACE FUNCTION public.begin_or_get_creative_director_run(
...
REVOKE ALL ON FUNCTION public.begin_or_get_creative_director_run(uuid,uuid,uuid,uuid,integer,uuid,integer,text,text,text,text,text,text,bigint,text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.begin_or_get_creative_director_run(uuid,uuid,uuid,uuid,integer,uuid,integer,text,text,text,text,text,text,bigint,text) TO service_role;
