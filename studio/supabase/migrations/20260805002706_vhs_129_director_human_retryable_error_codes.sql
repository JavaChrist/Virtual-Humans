-- VHS-129 — Human-retry allowlist for director_runs (additive).
-- Distinguishes human-confirmed retry from automatic/provider retry.
-- Single source of truth: director_error_code_is_human_retryable.
-- Legacy director_error_code_is_retryable delegates to it (compat wrapper).
-- No data mutations. service_role only.

BEGIN;

-- ---------------------------------------------------------------------------
-- Canonical human-retry allowlist (NOT auto-retry)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.director_error_code_is_human_retryable(p_code text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
...
COMMENT ON FUNCTION public.director_error_code_is_human_retryable(text) IS
  'VHS-129: codes eligible for explicit human retry only. Never used for automatic provider retry.';

-- Compat wrapper — same semantics, one source of truth
CREATE OR REPLACE FUNCTION public.director_error_code_is_retryable(p_code text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT public.director_error_code_is_human_retryable(p_code);
$$;

COMMENT ON FUNCTION public.director_error_code_is_retryable(text) IS
  'VHS-129 compat: delegates to director_error_code_is_human_retryable (human retry only).';

-- ---------------------------------------------------------------------------
-- begin_or_retry uses human-retryable gate
-- ---------------------------------------------------------------------------
...
REVOKE ALL ON FUNCTION public.director_error_code_is_human_retryable(text)
...
GRANT EXECUTE ON FUNCTION public.director_error_code_is_human_retryable(text) TO service_role;
