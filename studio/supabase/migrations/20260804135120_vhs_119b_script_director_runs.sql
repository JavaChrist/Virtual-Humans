-- VHS-119B — persisted Script Writer runs. Local additive migration only.
BEGIN;
...
ALTER TABLE public.director_runs ADD CONSTRAINT director_runs_type_check
  CHECK (director_type IN ('marketing', 'creative', 'script'));
...
ALTER TABLE public.director_runs ADD CONSTRAINT director_runs_input_type_check
  CHECK (input_artifact_type IN ('video_project_brief', 'marketing_plan', 'creative_concept'));

CREATE OR REPLACE FUNCTION public.begin_or_get_script_director_run(
...
BEGIN
  ...
  VALUES (p_id,p_workspace_id,p_project_id,'script','creative_concept',p_creative_concept_artifact_id,p_creative_concept_revision,'pending','openai',p_model_id,p_prompt_version,p_schema_version,p_idempotency_key,p_command_fingerprint,p_estimated_cost_minor,CASE WHEN p_estimated_cost_minor IS NULL THEN 'none' ELSE 'estimated' END,p_currency,p_correlation_id);
...
REVOKE ALL ON FUNCTION public.begin_or_get_script_director_run(uuid,uuid,uuid,uuid,integer,uuid,integer,uuid,integer,text,text,text,text,text,text,bigint,text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.begin_or_get_script_director_run(uuid,uuid,uuid,uuid,integer,uuid,integer,uuid,integer,text,text,text,text,text,text,bigint,text) TO service_role;
